...
import { giveAdvance } from '@/lib/erp/accounts';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const id = await giveAdvance(auth.email, { fromAccountId, toAccountId, amount: Number(amount), note: note || '', employeeId, createdBy: auth.email });
