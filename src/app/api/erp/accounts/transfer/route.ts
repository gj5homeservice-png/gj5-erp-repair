...
import { transferBetweenAccounts } from '@/lib/erp/accounts';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const id = await transferBetweenAccounts(auth.email, fromAccountId, toAccountId, Number(amount), note || '', auth.email);
