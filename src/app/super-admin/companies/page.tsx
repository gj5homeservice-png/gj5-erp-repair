...
export default function CompanyManager() {
  ...
  const companies = [
    { id: 'GJ5-1001', name: 'Master Apppliance Services', owner: 'Rahul Patel', email: 'owner@master.com', status: 'Active', plan: '12 Months', expiry: '2025-12-31', users: 12 },
    { id: 'GJ5-1002', name: 'Cool Care Solutions', owner: 'Amit Shah', email: 'amit@coolcare.in', status: 'Trial', plan: 'Free Trial', expiry: '2024-06-15', users: 2 },
    { id: 'GJ5-1003', name: 'Royal Tech Hub', owner: 'Vikram Singh', email: 'v@royal.co', status: 'Suspended', plan: '6 Months', expiry: '2024-08-10', users: 5 },
    { id: 'GJ5-1004', name: 'Surat Service Center', owner: 'Deepak Jain', email: 'surat@serv.com', status: 'Expired', plan: '3 Months', expiry: '2023-12-01', users: 8 },
  ];
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-6">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 text-[10px] uppercase font-bold tracking-[0.3em]">Lifecycle Management & Database Access Control</p>
        </div>
              ...
              <TableRow key={c.id} className="border-slate-800/40 hover:bg-white/5 transition-all group h-20">
                <TableCell className="px-8">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center font-black text-blue-500 border border-slate-700 shadow-inner group-hover:border-blue-500/50 transition-colors">
                      {c.name[0]}
                    </div>
                    <div className="flex flex-col">
                      <span className="font-headline font-bold text-sm text-slate-100 group-hover:text-blue-400 transition-colors">{c.name}</span>
                ...
                <TableCell>
                   <div className="flex flex-col">
                      <span className="text-xs font-bold text-slate-300">{c.owner}</span>
                      <span className="text-[10px] text-slate-500 italic">{c.email}</span>
                   </div>
                </TableCell>
                <TableCell>
                   <div className="flex flex-col">
                      <Badge variant="outline" className="text-[9px] uppercase border-slate-800 w-fit h-4 px-1.5 font-black text-blue-400">{c.plan}</Badge>
                      <span className="text-[9px] text-slate-500 font-bold uppercase mt-1">Exp: {format(parseISO(c.expiry), 'dd MMM yy')}</span>
                   </div>
                </TableCell>
                <TableCell>
                  <Badge className={cn(
                    ...
                    c.status === 'Suspended' ? "bg-rose-500/10 text-rose-400 animate-pulse" :
                ...
                <TableCell className="text-right px-8">
                  <div className="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-slate-500 hover:text-blue-400 hover:bg-blue-500/10" title="Login as Company"><LogIn className="w-4 h-4" /></Button>
