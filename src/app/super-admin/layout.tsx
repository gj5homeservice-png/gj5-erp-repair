...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 flex font-body overflow-hidden selection:bg-purple-500/30">
      {/* Premium Sidebar (desktop / tablet-landscape only — lg and up) */}
      <aside className={cn(
        "bg-slate-950/40 border-r border-slate-800/50 backdrop-blur-3xl transition-all duration-500 hidden lg:flex lg:flex-col z-50 shrink-0",
      ...
      {/* Mobile / Tablet Nav Drawer — same ADMIN_NAV content/routing as the desktop sidebar above */}
      <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
        <SheetContent side="left" className="w-72 p-0 bg-slate-950/95 border-slate-800/50 backdrop-blur-3xl flex flex-col z-50">
          ...
          <div className="p-4 border-t border-slate-800/50">
            <Button
              variant="ghost"
              className="w-full justify-start gap-4 text-slate-500 hover:text-rose-400 hover:bg-rose-500/5 h-14 rounded-2xl"
              onClick={() => router.push('/')}
            >
              <LogOut className="w-5 h-5" />
              <span className="font-bold text-xs uppercase tracking-widest">Destroy Session</span>
            </Button>
          </div>
        </SheetContent>
      </Sheet>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-20 border-b border-slate-800/50 flex items-center justify-between px-4 md:px-8 bg-slate-950/20 backdrop-blur-xl z-10 shrink-0 gap-3 flex-wrap sm:flex-nowrap">
          <div className="flex items-center gap-3 md:gap-6 min-w-0">
            <Button variant="ghost" size="icon" onClick={() => setMobileNavOpen(true)} className="lg:hidden text-slate-400 hover:text-white rounded-xl shrink-0">
              <Menu className="w-5 h-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => setSidebarOpen(!isSidebarOpen)} className="hidden lg:flex text-slate-400 hover:text-white rounded-xl shrink-0">
          ...
          <div className="flex items-center flex-wrap gap-3 sm:gap-6 shrink-0">
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-slate-800/50">
              ...
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 p-0.5 shadow-lg shrink-0">
