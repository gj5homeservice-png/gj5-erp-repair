...
interface RepairDeleteModalProps {
  isOpen: boolean;
  onClose: () => void;
  jobId: string;
  onConfirm: () => void;
...
export function RepairDeleteModal({ isOpen, onClose, jobId, onConfirm }: RepairDeleteModalProps) {
  ...
  const handleFinalConfirm = () => {
    try {
      ...
      toast({
        ...
        description: `Repair job ${jobId} removed from registry.`
      });
      resetAndClose();
    } catch (err) {
      toast({
        ...
        description: "An error occurred while deleting the repair job."
      });
    }
  };
